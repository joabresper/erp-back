...
import {
  CHECK_HIERARCHY_KEY,
  HierarchyEntity,
} from '../decorators/check-hierarchy.decorator';
...
export class HierarchyGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const entityType = this.reflector.get<HierarchyEntity>(
      CHECK_HIERARCHY_KEY,
      context.getHandler(),
    );
    ...
    if (entityType === 'user') {
      if (body?.roleId !== undefined) {
        targetLevel = Number(
          (await this.rolesService.findById(body.roleId)).level,
        );
        if (params.id !== undefined) {
          const oldLevel = Number(
            (await this.usersService.findByIdWithRole(params.id)).role.level,
          );
    ...
    if (entityType === 'role') {
      if (body?.level !== undefined) {
        ...
        if (params.id !== undefined) {
          const oldLevel = Number(
            (await this.rolesService.findById(params.id)).level,
          );
          targetLevel = oldLevel > targetLevel ? oldLevel : targetLevel;
        }
      } else {
